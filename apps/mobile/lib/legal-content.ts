...
export const PRIVACY_SECTIONS: LegalSection[] = [
  ...
  },
  {
    ...
    content: `When you use the Lenda mobile app, the app may ask for permission to send you push notifications so we can alert you about booking updates such as confirmations, handovers, and status changes. Granting this permission is optional: you may decline, and the core features of the app continue to work without it. If you decline, no device push token is collected. Your login session — the access and refresh tokens that keep you signed in — is stored using your device's secure, hardware-backed storage (the iOS Keychain or the Android Keystore, accessed through Expo SecureStore) and is not accessible to other apps on your device. If you enable notifications, a device push token is generated and associated with your account for the sole purpose of delivering booking-related alerts; it is never used for advertising. When you sign out, your stored session is cleared from the device and the device push token is deleted from our systems. You may also request deletion of any device token held against your account at any time using the contact details below.`,
  },
  {
    title: "Camera and Photo Access",
    content: `The Lenda mobile app requests access to your camera or photo library only at the moment you choose to upload an identity verification (KYC) document — never in the background — and solely to let you capture or select the specific image you are uploading. The images you upload are handled as KYC documents under the storage, retention, and security terms described above. Declining camera or photo library access means you cannot complete those specific upload actions, but you can continue to use the rest of the app normally. If profile photo upload is introduced in the app, the same rules will apply to it.`,
  },
  {
