...
export default function ListingsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["listings", pillar, category, location, page],
    queryFn: () =>
  ...
  const hasFilters = pillar || (category && category !== "All") || location;

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    setFilter("location", searchInput);
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex gap-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Mobile filters */}
            {showFilters && (
              <div className="lg:hidden glass-card p-4 border border-border mb-6 flex flex-wrap gap-2">
                {pillars.map((p) => (
                  <button
                    key={p.value}
                    onClick={() => setFilter("pillar", p.value)}
                      ...
                      <div className="glass-card border border-border hover:border-gold/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg overflow-hidden">
                        ...
                        <div className="p-4">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            <h3 className="font-display font-bold text-sm text-foreground uppercase tracking-tight leading-tight truncate">
                              {listing.title}
                            </h3>
                          </div>
